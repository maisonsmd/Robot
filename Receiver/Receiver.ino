...
#define motor_init_isr(motor) \
		motor.m_timer_on->pause();\
		motor.m_timer_on->attachInterrupt(0, []() {\
			motor.isr_on();\
		});\
 ...
 // Timer2.refresh();
  ...
  stepper_left.setAccel(ACCEL);
  stepper_right.setAccel(ACCEL);
}
...
void loop() {
  ...
  if (Serial.available()) {
	...
	if (c == 'a') {
	  stepper_left.setAccel(v);
	  stepper_right.setAccel(v);
	}
	if (c == 'v') {
	  stepper_left.setVelocity(v);
	  stepper_right.setVelocity(v);
	}
